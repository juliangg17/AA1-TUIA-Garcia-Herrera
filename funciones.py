...
def update_weights(x_sample, y_sample, W, lr):
    ...
    new_W = W - (lr * gradient)
    return new_W, error_train

def stochastic_gradient_descent(X_train, y_train, X_test, y_test, lr=0.01, epochs=100, n_jobs=-1):
    n = X_train.shape[0]
    m = X_train.shape[1]
    ...
    for epoch in tqdm(range(epochs), desc='Training Epochs', unit='epoch'):
        ...
        results = Parallel(n_jobs=n_jobs)(delayed(update_weights)(
            X_train_permuted[j], y_train_permuted[j], W, lr) for j in range(n))

        W, errors_train_epoch = zip(*results)
        W = np.mean(W, axis=0)  # Promediar los pesos actualizados
        train_rmse = np.sqrt(np.mean(np.square(errors_train_epoch)))
        train_errors.append(train_rmse)

        prediction_test = np.dot(X_test, W)
        error_test = y_test - prediction_test
        test_rmse = np.sqrt(np.mean(error_test ** 2))
        test_errors.append(test_rmse)

    prediction_train_final = np.dot(X_train, W)
    prediction_test_final = np.dot(X_test, W)
...
# Definir métrica personalizada para f1 ponderado
@tf.function
def f1_weighted(y_true, y_pred):
    ...
    total_f1 = tf.constant(0.0)
    total_count = tf.constant(0.0)
    
    for i in tf.range(tf.size(unique_labels)):
        cls = tf.cast(unique_labels[i], tf.int32)
        cls_count = tf.cast(unique_count[i], tf.float32)
        ...
        total_f1 += f1 * cls_count
        total_count += cls_count
    
    weighted_f1 = total_f1 / total_count
    return weighted_f1

# Definir métrica personalizada para recall ponderado
@tf.function
def recall_weighted(y_true, y_pred):
    ...
    for i in tf.range(tf.size(unique_labels)):
        ...
        total_recall += recall * cls_count
        total_count += cls_count
    
    weighted_recall = total_recall / total_count
